...
class XiaohongshuPublisher:
    ...
    def _click_image_text_tab(self):
        ...
        try:
            ...
            fallback_clicked = self._evaluate("""
                (() => {
                    const patterns = [/上传图文/, /图文/, /发布图文/, /发布笔记/, /上传内容/];
            ...
            """)
            if fallback_clicked and fallback_clicked.get("ok"):
                print(
                    f"[cdp_publish] Fallback clicked image-text tab candidate: {fallback_clicked.get('clicked')}"
                )
                self._sleep(TAB_CLICK_WAIT, minimum_seconds=0.8)
                return
            ...
            if upload_ready:
                print(
                    "[cdp_publish] Image-text tab not found, but upload input is ready. Continuing..."
                )
                return
